...
def escribir_contenido():
    ...
    with open('Python/Tareas segundo semestre/Archivos/ejemplo.txt','w') as e:
        for i in range(3):
            e.write(f"{contenido[i]}\n")
    with open('Python/Tareas segundo semestre/Archivos/ejemplo.txt','r') as r:
        c = r.readlines()
    with open('Python/Tareas segundo semestre/Archivos/nuevo_ejemplo.txt','w') as w:
        for i in range(-2,0):
            w.write(f"{c[i]}")
escribir_contenido()
